...
document.addEventListener("DOMContentLoaded", () => {
    ...
    /* ─────────────────────────────────────────────
       PROPUESTA 5 — Apertura cinematográfica de lente
    ───────────────────────────────────────────── */
    (() => {
        const overlay = document.getElementById('lens-overlay');
        const titulo  = document.querySelector('.titulo-elegante');
        if (!overlay) return;
        
        const reducido = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
        if (reducido) {
            overlay.style.display = 'none';
            if (titulo) titulo.classList.add('lens-visible');
            return;
        }
        ...
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                const shimmer = overlay.querySelector('.lens-shimmer');
