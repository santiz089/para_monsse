...
const esMobil = () => window.matchMedia("(max-width: 768px)").matches || ('ontouchstart' in window);
...
btnNext.addEventListener("touchend", e => {
    ...
    async function coreografiaSmartphone() {
        ...
        const msg1 = $("msg-1"), msg2 = $("msg-2"), msg3 = $("msg-3"), typing = $("sp-typing");

        if (!sp) return;

        const scrollDown = () => { if (scrollContainer) setTimeout(() => { scrollContainer.scrollTop = scrollContainer.scrollHeight; }, 50); };
        ...
        await mostrarMensaje(msg1, 2000); if (signal.aborted) return;
        await delay(1200); if (signal.aborted) return;
        await mostrarMensaje(msg2, 2500); if (signal.aborted) return;
        await delay(1200); if (signal.aborted) return;
        await mostrarMensaje(msg3, 1800); if (signal.aborted) return;
                ...
                flipTimeout = setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            escribirTextoCore("texto-0", "Desde que llegaste a mi vida, cada pequeño instante a tu lado se ha convertido en mi recuerdo favorito.", 60).then(() => {
                                escribirTextoCore("firma-typing", "Luis Angel", 120);
                            });
                        }, 2000);
                    }, 400);
                }, 2800);
